  ...
  return (
    <div className="min-h-full px-3 py-4 sm:px-6 sm:py-6 lg:px-8 lg:py-8">
      ...
      <div className="mx-auto max-w-6xl">
        <header className="page-header">
          {selectedElectionId && (
            <button
              ...
              className="mb-3 inline-flex items-center gap-1.5 text-sm font-medium text-surface-500 transition-colors hover:text-surface-900 dark:text-surface-400 dark:hover:text-surface-100"
            >
        ...
        {!selectedElectionId ? (
          loading ? <OverviewLoadingState /> : elections.length === 0 ? (
            <div className="card py-14 text-center"><ClipboardDocumentCheckIcon className="mx-auto h-12 w-12 text-surface-300 dark:text-surface-700" /><h2 className="mt-4 text-lg font-semibold">Keine offenen Wahlen</h2><p className="mt-1 text-sm text-surface-500">Für dein Konto gibt es derzeit keine verfügbare Wahl.</p></div>
          ) : (
            <div className="grid gap-4 md:grid-cols-2">
              {elections.map(election => (
                <article key={election.id} className="card card-hover !p-0 overflow-hidden">
                  <button type="button" onClick={() => openElection(election.id)} className="flex w-full items-center gap-4 p-5 text-left sm:p-6">
                    <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-primary-50 text-primary-600 dark:bg-primary-950/50 dark:text-primary-400">
                      <ClipboardDocumentCheckIcon className="h-6 w-6" />
                    </span>
                    <span className="min-w-0 flex-1">
                      <span className="block font-semibold text-surface-900 dark:text-surface-100">{election.title}</span>
                      <span className="mt-1 block text-sm leading-5 text-surface-500 dark:text-surface-400">{election.participation_text || 'Formular öffnen und Angaben prüfen'}</span>
                    </span>
          ...
          <form onSubmit={openReview} className="space-y-6">
            ...
            <div className="flex flex-col items-stretch justify-between gap-4 rounded-2xl border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900 sm:flex-row sm:items-center sm:p-5"><div className="text-sm text-surface-500 dark:text-surface-400">{validationErrors.length > 0 ? `${validationErrors.length} Angaben fehlen noch.` : 'Alles ausgefüllt? Dann prüfe deine Angaben im nächsten Schritt.'}</div><button type="submit" className="btn btn-primary inline-flex justify-center gap-2" disabled={validationErrors.length > 0}><span>Angaben prüfen</span><ChevronRightIcon className="h-4 w-4" /></button></div>
          </form>
        )}
      </div>
...
const OverviewLoadingState: React.FC = () => (
  <div className="grid gap-4 md:grid-cols-2">
...
const FormLoadingState: React.FC = () => (
  <div className="space-y-4">
    ...
    <div className="flex items-center justify-center gap-2 py-3 text-sm text-surface-500">
